...
int BTree<keyType>::Remove (const keyType key, const int recAddr)
{
	...
	BTNode * thisNode, * siblingNode, * parentNode;

	// Find leaf node
	thisNode = FindLeaf (key);

	// case 1 : The key being removed is the largest at that level.
	if(key == thisNode -> LargestKey())
	{
	...
	// case 2: The key being removed is not the largest at that level.
	else if(key < thisNode -> LargestKey())
		result = thisNode -> Remove(key, recAddr);
	...
	while (result==-1)
	{
		...
		// Merge with the sibling node
		siblingNode = Nodes[thisNode->NextNode];
		thisNode -> Merge(siblingNode);
		Store(thisNode);
		siblingNode -> Clear();
		...
		// Update parent with new largest key
		parentNode = Nodes[level];
		result = parentNode -> UpdateKey
			(largestKey, thisNode->LargestKey());
		thisNode=parentNode;
