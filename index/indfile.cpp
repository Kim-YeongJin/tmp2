...
template <class RecType>
int TextIndexedFile<RecType>::Remove (char * key, RecType & record)
{
	int ref;
	ref = Index.Remove(key);      // remove 할 key 
	if (!ref)                     // remove 할 key가 없음.
		return 0;
	if(NumKeys<MinKeys)           // 모든 key 검색끝
		return -1;
	int result = DataFile.Remove(record, ref); // remove  및 결과 반환
	return result;                
